...
- [ ] **1.1.1** Create concrete page types in router instead of `tea.Model`
...
- [ ] **1.2.6** Run build and fix compilation errors
  - Command: `go build ./...`
...
- [ ] **1.3.1** Audit current message usage
  - File: `pkg/ui/core/messages.go`
  - Identify all `DataLoadedMsg`, `DataErrorMsg` usages

- [ ] **1.3.2** Create specific message types for each data type
  ```go
  // Replace this:
...
- [ ] **1.3.3** Create specific error message types
  ```go
...
- [ ] **1.3.7** Run tests to verify message handling
  - Command: `go test ./pkg/ui/... -v`
...
- [ ] **1.4.1** Define UI state types
  - File: `pkg/ui/core/state.go` (new file)
  ```go
  type UIState uint8
...
- [ ] **1.4.2** Add state fields to root Model
  - File: `pkg/ui/ui.go`
  ```go
  type Model struct {
      state  UIState
...
- [ ] **1.4.3** Create state transition function
  - File: `pkg/ui/ui.go`
  ```go
...
- [ ] **1.4.4** Update all state checks to use new state types
  - Change: `if m.ShowHelp` → `if m.state == StateHelp`
  - Change: `if m.searchMode` → `if m.state == StateSearch`

- [ ] **1.4.5** Update state transitions in Update method
...
- [ ] **2.1.1** Create centralized key bindings file
  - File: `pkg/ui/keys/keys.go` (new directory)
  ```go
  package keys
...
- [ ] **2.1.2** Define all global key bindings
  - File: `pkg/ui/keys/keys.go`
  - Include: Quit, Help, Back, Search, Navigation

- [ ] **2.1.3** Define all page-specific key bindings
  - Organize by page: Main, Topic, Detail, ResourceDetail

- [ ] **2.1.4** Create helper function for help views
  ```go
...
- [ ] **2.2.1** Create Common context struct
  - File: `pkg/ui/core/common.go` (new file)
  ```go
  type Common struct {
...
- [ ] **2.2.2** Create Common constructor
...
- [ ] **2.3.2** Create layout calculator
  ...
  ```go
  func CalculateLayout(width, height int, state UIState, showSidebar bool) Layout
  ```
...
- [ ] **2.3.4** Update WindowSizeMsg handling
  ...
  case tea.WindowSizeMsg:
      ...
      m.layout = layout.CalculateLayout(msg.Width, msg.Height, m.state, showSidebar)
      m.propagateLayout()
  ```
...
- [ ] **2.3.5** Create layout propagation method
  - File: `pkg/ui/ui.go`
  ```go
  func (m *Model) propagateLayout() {
      m.Router.SetDimensions(m.layout.Main.Width, m.layout.Main.Height)
...
- [ ] **2.4.1** Define component interface
  ...
  type Component interface {
      SetCommon(com *Common)
      SetSize(width, height int)
      SetStyles(styles *Styles)
      HandleKeyMsg(msg tea.KeyMsg) (bool, tea.Cmd)
      Render() string
  }
  ```
...
- [ ] **2.4.2** Create component base struct
  ...
  func (b *BaseComponent) SetCommon(com *Common) { b.com = com }
...
- [ ] **3.1.1** Define semantic color palette
  - File: `pkg/ui/styles/colors.go` (new file)
  ```go
  var (
...
- [ ] **3.1.4** Define component-specific styles
  - File: `pkg/ui/styles/styles.go`
  ```go
...
- [ ] **3.1.5** Create DefaultStyles constructor
  - File: `pkg/ui/styles/styles.go`
  ```go
...
- [ ] **3.1.6** Update Common to include Styles
  - File: `pkg/ui/core/common.go`
...
|-------|-------------|-----------|------------|
| Phase 1: Foundation | 27 | 0 | 0% |
| Phase 2: Organization | 28 | 0 | 0% |
| Phase 3: Styling | 16 | 0 | 0% |
| Phase 4: Error Handling | 12 | 0 | 0% |
| Phase 5: Testing & Docs | 11 | 0 | 0% |
| Phase 6: Cleanup | 9 | 0 | 0% |
| **Total** | **103** | **0** | **0%** |

### Completion Checklist

- [ ] Phase 1 complete (all 27 tasks)
- [ ] Phase 2 complete (all 28 tasks)
- [ ] Phase 3 complete (all 16 tasks)
- [ ] Phase 4 complete (all 12 tasks)
- [ ] Phase 5 complete (all 11 tasks)
- [ ] Phase 6 complete (all 9 tasks)
...
### Critical Path
1. Phase 1 must be completed first (foundation changes)
2. Phase 2 depends on Phase 1 (can't organize what's not type-safe)
3. Phase 3 is independent but easier after Phase 2
...
- Phase 3: 1-2 days
- Phase 4: 1 day
- Phase 5: Ongoing (1-2 days total)
- Phase 6: 1 day
- **Total**: 8-12 days
...
2. **Start with Phase 1.1**: Eliminate model casting (highest impact)
