...
func TestShowNotification(t *testing.T) {
	t.Skip("Skipping notification test as of now")
	tp, _, app := createTestTopicPage()
	tp.notifyView = tview.NewTextView()

	// Test notification
	tp.ShowNotification("Test notification")

	// Process any queued updates by running the app's draw function
	// This ensures the QueueUpdateDraw callback is executed
	app.QueueUpdateDraw(func() {
		// This will process any previously queued updates
	})

	// Give some time for the goroutine to execute and queue the update
	time.Sleep(100 * time.Millisecond)

	// Process the queued update again to ensure it's applied
	app.QueueUpdateDraw(func() {
...
func TestPageConsumeTopic(t *testing.T) {
	t.Skip("Skipping TestPageConsumeTopic test as of now")
